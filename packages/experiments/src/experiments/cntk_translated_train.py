...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(256, 10)  # Adjust based on input size
...
def main():
    ...
    if args.save_model:
        torch.save(model.state_dict(), "mnist_cnn_CNTK_translated2.pt")
        example_inputs = (torch.randn(1, 1, 28, 28),)
        onnx_program = torch.onnx.export(model, example_inputs, dynamo=True)
        onnx_program.optimize()
        onnx_program.save("mnist_cnn_CNTK_translated2.onnx")
