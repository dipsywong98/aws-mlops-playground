...
def lambda_handler(event, context):
    ModelPackageName = event.get("queryStringParameters", {}).get(
    ...
    if ModelPackageName is None or ModelPackageName == "":
        ...
        return resp
    try:
        res = sm_client.describe_model_package(ModelPackageName=ModelPackageName)
    except Exception as e:
        print(f"Error describing model package: {e}")
        return {"statusCode": 404, "body": "Model package not found"}
